...
def create_chart(input_data, chart_type):
    try:
        ...
        # 由时候大模型会犯傻，所以写一个转换，防止出错
        # 处理 data 为字典的情况（转换为列表格式：[[key1, value1], [key2, value2], ...]）
        if isinstance(data, dict):
            # 将字典转换为列表，例如 {"a":1, "b":2} → [["a",1], ["b",2]]
            data = list(data.items())
            st.info("检测到字典格式数据，已自动转换为列表格式")

        # 校验 data 是否为列表（确保能转换为 DataFrame）
        if not isinstance(data, list) or len(data) == 0:
            st.error("图表数据 'data' 必须是非空列表（或可转换为列表的字典）")
            return

        # 创建DataFrame（确保columns与data结构匹配）
        # 处理data中元素长度不一致的情况 有时部分元素缺少值
        df_data = pd.DataFrame(data, columns=columns)

        # 校验DataFrame结构（确保至少有两列：x轴和y轴）
        if len(df_data.columns) < 2:
            st.error("图表数据至少需要两列（x轴和y轴数据）")
            return

        # 设置索引为x轴 columns[0]通常是类别/标签列
        df_data.set_index(columns[0], inplace=True)

        # 根据图表类型渲染 使用Streamlit原生图表函数
        if chart_type == "bar":
        ...
        st.info(f"问题数据：columns={columns}, data={data}")
